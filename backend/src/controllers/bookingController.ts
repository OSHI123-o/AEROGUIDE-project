...
import { z } from "zod";
import { findFlights, findPassenger, type BookingLookupResponse } from "../models/bookingModel.js";
...
export async function bookingLookup(req: Request, res: Response) {
  ...
  try {
    ...
    const response: BookingLookupResponse = { passenger, flights };
...
export async function flightLookup(req: Request, res: Response) {
  const parsed = BookingLookupSchema.safeParse(req.body);
  if (!parsed.success) {
...
export async function passengerSuggest(req: Request, res: Response) {
