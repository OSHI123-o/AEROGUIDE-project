...
function rowValue(row: RawRow, camelKey: string, snakeKey: string) {
  const camel = row[camelKey];
  if (camel !== undefined && camel !== null && camel !== "") return asText(camel);
  return asText(row[snakeKey]);
}

function normalizePassenger(row: RawRow): Passenger {
  return {
    ...
    cabin: (rowValue(row, "cabin", "cabin") as Cabin) || undefined,
  };
}

function normalizeFlight(row: RawRow): Flight {
  const status = rowValue(row, "status", "status");
  const safeStatus: Flight["status"] =
    status === "Boarding Soon" || status === "Delayed" ? status : "On Schedule";
...
async function querySupabase(table: string, pnr: string, lastName: string, limit?: number) {
...
export async function findPassenger(pnr: string, lastName: string) {
  const env = getEnvConfig();
  if (env.useSupabase) {
    try {
      const rows = await querySupabase(env.supabasePassengersTable, pnr, lastName, 1);
      if (rows && rows.length) return normalizePassenger(rows[0]);
    } catch {
...
export async function findFlights(pnr: string, lastName: string) {
  const env = getEnvConfig();
  if (env.useSupabase) {
    try {
      const rows = await querySupabase(env.supabaseFlightsTable, pnr, lastName);
      if (rows && rows.length) return rows.map(normalizeFlight);
    } catch {
      // Fall back to demo data when Supabase is unreachable/misconfigured.
    }
  }
