...
export default function BoardingPassPage() {
  ...
  const bg = themeMode === "light"
    ? "linear-gradient(165deg,#f9fbff 0%,#fff6dd 45%,#e9f1ff 100%)"
    : "linear-gradient(165deg,#061227 0%,#0a1f3d 45%,#133761 100%)";

  const textMain = themeMode === "light" ? "#002B5B" : "#e2e8f0";
  const textMuted = themeMode === "light" ? "#52627a" : "#b3c1d8";
  const panel = themeMode === "light" ? "rgba(255,255,255,0.8)" : "rgba(8,23,46,0.72)";
  const panelBorder = themeMode === "light" ? "rgba(0,43,91,0.2)" : "rgba(253,185,19,0.22)";
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, padding: 24, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 90, left: 80, width: 180, height: 180, borderRadius: "50%", background: "rgba(253,185,19,0.28)", filter: "blur(50px)", animation: "floatY 6s ease-in-out infinite" }} />
      <div style={{ position: "absolute", bottom: 80, right: 70, width: 220, height: 220, borderRadius: "50%", background: "rgba(0,43,91,0.24)", filter: "blur(50px)", animation: "floatY 7s ease-in-out infinite" }} />

      <div style={{ maxWidth: 980, margin: "0 auto", position: "relative", zIndex: 2 }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div style={{ color: textMain }}>
            <div style={{ fontSize: 32, fontWeight: 800 }}>Flight Journey</div>
            <div style={{ color: textMuted }}>AEROGUIDE journey details, status, and navigation</div>
          </div>
        ...
        >
          <div style={{ padding: 20, display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 10 }}>
            <div>
              <div style={{ fontSize: 64, fontWeight: 900, lineHeight: 1, color: "#002B5B" }}>{flight.originCode}</div>
              <div style={{ marginTop: 8, color: textMuted, fontSize: 26 }}>{flight.originCity}</div>
            </div>
            <div style={{ fontSize: 38, color: textMuted, animation: "movePlane 3s ease-in-out infinite" }} aria-label="Flight icon">
              {"\u2708\uFE0F"}
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 64, fontWeight: 900, lineHeight: 1, color: "#002B5B" }}>{flight.destinationCode}</div>
              <div style={{ marginTop: 8, color: textMuted, fontSize: 26 }}>{flight.destinationCity}</div>
            </div>
          </div>

          <div style={{ background: "#002B5B", color: "#fff", display: "grid", gridTemplateColumns: "1fr 1fr 1fr" }}>
          ...
          <div style={{ padding: 18, display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr 1fr", gap: 10 }}>
            <div style={{ border: "2px dashed #002B5B", borderRadius: 12, padding: 12, background: "rgba(255,255,255,0.58)" }}>
              <div style={{ fontSize: 13, color: "#334155", fontWeight: 700 }}>PNR</div>
              <div style={{ marginTop: 2, fontSize: 42, fontWeight: 900, color: "#002B5B", letterSpacing: 1 }}>{flight.pnr}</div>
            </div>
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <div style={{ minWidth: 220 }}>
              <div style={{ height: 8, borderRadius: 999, background: "rgba(148,163,184,0.35)", overflow: "hidden" }}>
                <div style={{ width: `${progress}%`, height: "100%", background: "#FDB913" }} />
              </div>
          ...
          <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
            {steps.map((s, idx) => (
              <div
                ...
                style={{
                  borderRadius: 10,
                  border: `1px solid ${idx === stepIndex ? "#FDB913" : panelBorder}`,
                  background: idx <= stepIndex ? "rgba(253,185,19,0.14)" : "rgba(255,255,255,0.58)",
                  padding: "10px 12px",
                  display: "flex",
              ...
              >
                <div>
                  <div style={{ fontWeight: 700, color: textMain }}>{idx + 1}. {s.title}</div>
                  <div style={{ color: textMuted, fontSize: 13 }}>{s.detail}</div>
                </div>
                <div style={{ fontSize: 12, fontWeight: 700, color: idx < stepIndex ? "#16a34a" : idx === stepIndex ? "#002B5B" : textMuted }}>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button
              ...
              style={{ border: `1px solid ${panelBorder}`, borderRadius: 10, padding: "10px 14px", background: "#fff", color: "#002B5B", fontWeight: 700, cursor: "pointer" }}
            >
              Mark Step Done
            </button>
            <button
              onClick={() => navigate("/guide")}
              style={{ border: `1px solid ${panelBorder}`, borderRadius: 10, padding: "10px 14px", background: "rgba(255,255,255,0.8)", color: "#002B5B", fontWeight: 700, cursor: "pointer" }}
            >
              Open Full Guide
            </button>
          </div>
...
function InfoCard({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ borderRadius: 10, border: "1px solid rgba(0,43,91,0.2)", background: "rgba(255,255,255,0.75)", padding: 10 }}>
      <div style={{ fontSize: 12, color: "#64748b" }}>{label}</div>
      <div style={{ marginTop: 4, fontSize: 19, fontWeight: 800, color: "#002B5B" }}>{value}</div>
    </div>
  );
}
